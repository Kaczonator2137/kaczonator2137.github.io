(() => {
  ...
  async function bootstrap(){
    try{
      ...
      setTimeout(maybePromptNotifications,350);
  ...
  function maybePromptNotifications(){const dialog=$('#notificationPrompt');if(!dialog||dialog.open)return;const installed=window.matchMedia('(display-mode: standalone)').matches||navigator.standalone===true;if(!installed||!('Notification'in window)||Notification.permission==='granted')return;if(Notification.permission==='denied'){$('#notificationPromptTitle').textContent='Powiadomienia są wyłączone';$('#notificationPromptText').textContent='Otwórz Ustawienia telefonu → Powiadomienia → CalisLevel i zezwól na alerty oraz dźwięki.';$('#notificationPromptEnable').classList.add('hidden');$('#notificationPromptLater').textContent='Rozumiem';}else{$('#notificationPromptTitle').textContent='Włącz powiadomienia';$('#notificationPromptText').textContent='Dostaniesz alert z dźwiękiem, gdy przerwa między seriami się zakończy — także po przełączeniu na inną aplikację.';$('#notificationPromptEnable').classList.remove('hidden');$('#notificationPromptLater').textContent='Może później';}dialog.showModal();}
  async function enableNotifications(){if(!('Notification'in window))return;try{if(Notification.permission==='default')await Notification.requestPermission();renderNotificationStatus();if($('#notificationPrompt').open)$('#notificationPrompt').close();if(Notification.permission==='granted'){await showNotification('Powiadomienia włączone','Damy znać, gdy przerwa się zakończy.','calislevel-test');toast('Powiadomienia są włączone');}else{toast('Powiadomienia pozostają wyłączone');}}catch{toast('Nie udało się włączyć powiadomień');}}
  $('#enableNotificationsBtn').onclick=enableNotifications;
  $('#notificationPromptEnable').onclick=enableNotifications;
  $('#notificationPromptLater').onclick=()=>$('#notificationPrompt').close();
