(() => {
  ...
  function openPlanEditor(plan=null){
    ...
    openModal(plan?'Edytuj plan':'Nowy plan',()=>savePlan(plan?.id),`<label>Nazwa<input id="mPlanName" value="${esc(plan?.name||'')}"></label><label>Opis<input id="mPlanDesc" value="${esc(plan?.description||'')}"></label><div id="mPlanItems"></div><button type="button" class="secondary full" id="mAddPlanExercise">+ Dodaj ćwiczenie</button>`);
    renderPlanModalItems(); $('#mAddPlanExercise').onclick=()=>openExercisePicker(ex=>{state.selectedPlanExercises.push({exercise_id:ex.id,exercise:ex,sets:3,rep_min:8,rep_max:12,rest_sec:90}); renderPlanModalItems();});
  }
